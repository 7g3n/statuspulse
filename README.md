...
**Phase 2 完了。** 監視対象の登録と Cloudflare Workers による定期チェック（Phase 1）、ダウンタイム履歴・連続N回失敗での判定・Slack 通知（Phase 2）が動作する。
...
DB 側の不変条件（継続中インシデントは1本まで、観測ログは書き換え不能、定期処理用の関数は service_role 限定）は、ローカル Supabase に対して手で確認している。Phase 3 でテスト用 DB に対する統合テストとして自動化する予定。
