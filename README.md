...
**全4フェーズ完了。** 監視対象の登録と Cloudflare Workers による定期チェック（Phase 1）、ダウンタイム履歴・連続N回失敗での判定・Slack 通知（Phase 2）、公開ステータスページとチームでの共有（Phase 3）、本文チェック・TLS 証明書の期限・ポストモーテム（Phase 4）が動作する。
...
1日1回確認し、残り30日と7日で通知する。Cloudflare Workers の `fetch()` は相手の証明書を見せてくれないので、TLS ハンドシェイクを自前で書いて取得している（後述）。

![TLS 証明書](docs/images/07-certificate.png)
...
| `pnpm test`      | 判定ロジックのテスト（DB 不要）    |
| `pnpm test:db`   | DB の不変条件のテスト（要 Docker） |
...
│  └─ db-tests/                DB の不変条件の統合テスト（pnpm test:db）
├─ supabase/
...
Phase 1〜3 は「到達できるか」を見てきた。Phase 4 で足したのは、**到達できていても壊れている**状態を捉えるための2つになる。

### レスポンス本文のチェック

ステータスコードだけを見ていると、アプリが落ちて前段のプロキシが 200 でメンテナンス画面を返している状態や、デプロイ失敗で中身が空になっている状態を取りこぼす。

正規表現ではなく「含まれているか」だけにしてある。正規表現は書き手が意図しない量の計算を招きうる（Worker の実行時間を食う）うえ、監視の設定としては「この文字列が消えたら異常」で足りることがほとんどになる。

**本文を読むのは、設定がある対象だけ。** 毎回すべて読むと、監視のためだけに全対象ぶんの転送量と Worker の実行時間を消費し続けることになる。読む場合も 512KB で打ち切る。

判定はステータスの後に行う。500 が返っているときに「本文が違う」と報告しても、原因の手がかりにならない。

### TLS 証明書の期限

**Cloudflare Workers の `fetch()` は相手の証明書を見せてくれない。** Node の `tls` モジュールも Workers には無く、この実行環境には「証明書の期限を知る」手段が用意されていない。

外部の証明書チェック API を使う手もあったが、監視ツールが別のサービスの可用性に依存することになる。「監視が落ちていて気付けなかった」の原因を増やしたくない。

そこで `cloudflare:sockets` で生の TCP を開き、**TLS ハンドシェイクを自前で書いて**証明書を読んでいる。

- **TLS 1.2 を使う** — TLS 1.3 では Certificate メッセージが暗号化されるため、鍵交換まで実装しないと読めない。TLS 1.3 は `supported_versions` 拡張がある場合にのみ選ばれるので、その拡張を送らなければ 1.2 以下が選ばれる
- **ハンドシェイクは完了させない** — 証明書を受け取った時点で目的は果たされる。鍵交換も署名の検証もしない
- **解析は純粋関数にした** — ClientHello の組み立てと X.509（DER）の解析は [`packages/core/src/tls.ts`](./packages/core/src/tls.ts) にあり、ソケットに触れない。UTCTime の2桁の年（RFC 5280 では 50 以上が 19xx）のような、間違えると70年ずれる箇所はテストで固定してある

検査は到達性のチェックと分けて1日1回にしている。有効期限は数か月単位で動くもので、同じ処理に混ぜると**毎分の TLS ハンドシェイクを監視先に強いる**ことになる。

取得に失敗したら、期限を前回の値のまま残さず失敗として記録する。取得できないこと自体が知りたい情報で、黙って残すと「期限は大丈夫」と誤読される。

通知の重複を防ぐ鍵には**証明書の期限そのもの**を含める。更新されれば鍵が変わるので次の通知が届き、更新されない限り二度は鳴らない。

### ポストモーテム

稼働率も失敗回数も、**なぜ落ちたか**は教えてくれない。同じ原因を三度目に踏むかどうかは、そこが書かれているかで決まる。

**「書く」と「公開する」を別のフラグにしてある。** 社内向けの生々しいメモと、利用者向けの説明は普通は別物になる。1つにすると「公開したくないから書かない」が起きる。書く障壁を下げることの方が、公開の手軽さより価値がある。

公開が許可されたものは公開ステータスページにも載る。「9月17日 30分停止」だけでは、見た人は何も分からないままになる。
...
| 本文チェック       | 含まれる／含まれない、ステータス異常との判定順序                               |
| TLS の組み立て     | ClientHello の形、supported_versions を送らないこと                            |
| 証明書の解析       | UTCTime の2桁の年、GeneralizedTime、長さの長形式、version 省略、壊れた入力     |
| 証明書の期限       | 30日 / 7日の境界、残り日数の切り捨て、鍵が証明書の更新でだけ変わること         |

**DB 側の不変条件は別のスイートにしてある。**
...
観測ログが書けないこと、判定のキャッシュ列が UPDATE できないこと、継続中インシデントが1本であること、通知が二度 claim できないこと、役割ごとに操作が分かれていること、`anon` がどのテーブルにも触れず `public_status()` だけが通ること、公開ページの応答に監視先 URL やエラー本文が含まれないこと——を確かめている。

`pnpm test` に混ぜないのは、こちらが Docker を要するため。判定ロジックのテストが DB 無しで数百ミリ秒で終わることには、それ自体に価値がある。壊れたときの被害も維持コストも違うものを、同じコマンドに入れない。

接続は supabase-js ではなく `pg` で行う。確かめたいのは「アプリを経由しない書き込みも塞がれているか」で、supabase-js は PostgREST という1つの経路の話でしかない。各テストはトランザクションで包んでロールバックするので、seed のデータをそのまま前提にでき、実行順にも依存しない。
...
- [x] **Phase 4** — TLS 証明書の期限チェック、レスポンス本文の文字列チェック、ポストモーテム、DB の不変条件の統合テスト
